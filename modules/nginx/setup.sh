...
if [ -n "$TARGET_VERSION" ]; then
    ...
    # 使用環境變數 VERSION_DB (由 engine.sh 提供)
    if [ -f "$VERSION_DB" ]; then
        VERSION=$(grep "^nginx|" "$VERSION_DB" | cut -d'|' -f2)
    fi
...
# 4. 匯入 Nginx 官方 GPG 金鑰 (加上 --batch --yes 避免互動詢問)
sudo install -m 0755 -d /etc/apt/keyrings
curl -fsSL https://nginx.org/keys/nginx_signing.key | sudo gpg --batch --yes --dearmor -o /etc/apt/keyrings/nginx-archive-keyring.gpg
